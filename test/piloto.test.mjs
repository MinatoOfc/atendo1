...
test('sem ATENDO_LIBERAR_AUTOENVIO: loja neutralizada, agendamento antigo do novo cancelado, clássico intacto, nada sai', async () => {
  ...
  assert.equal(c2.status, 'aprovacao'); assert.equal(c2.enviaEm, undefined, 'agendamento vencido cancelado no piloto'); assert.equal(c2.atendimentoNovo.etapa, 'reemb_40', 'a confirmação não saiu'); assert.equal(c2.relatorioAuto, undefined); assert.equal(c2.atendimentoNovo.conclusaoPendente.status, 'aguardando_cadencia'); assert.equal(c2.atendimentoNovo.transicaoPendente.para, 'conf_reembolso')
