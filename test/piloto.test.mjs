...
test('sem ATENDO_LIBERAR_AUTOENVIO: loja neutralizada, agendamento antigo do novo cancelado, clássico intacto, nada sai', async () => {
  ...
  // a continuação virou MANUAL: rascunho/transição antigos invalidados (o clique do dono regera) e relatório automático proibido
  assert.equal(c2.atendimentoNovo.conclusaoPendente.modo, 'manual'); assert.equal(c2.atendimentoNovo.conclusaoPendente.modoOriginal, 'automatico'); assert.equal(c2.atendimentoNovo.conclusaoPendente.relatorioAutomaticoProibido, true)
  assert.equal(c2.atendimentoNovo.transicaoPendente, null); assert.equal(c2.rascunho, undefined); assert.equal(c2.atendimentoNovo.acaoAceita, 'reemb_40')
