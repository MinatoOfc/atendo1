...
estado.tickets = [
  ...
  { id: 'sp10', nome: 'Antigo 81', de: 'c81@web.de', assunto: 'Bestellung #81', corpo: 'Wo ist mein Paket mit dem Polo Premium?', data: new Date(Date.now() - 5 * 86400_000).toISOString(), primeiroEmailEm: new Date(Date.now() - 5 * 86400_000).toISOString(), lido: true, origem: 'cliente', categoria: 'rastreio', status: 'aprovacao', idioma: 'de', lojaId: 'loja1', historico: [], motor: 'classico', motorAtendimento: 'classico', rascunho: 'Ihr Paket ist unterwegs.', geradoPorIA: true },
  ...
  antigo('h903', { categoria: 'entrega', corpo: 'Wo ist mein Paket mit dem Polo Premium?' }),
  ...
  antigo('h907', { categoria: 'reembolso', corpo: 'Ja, 60% ist ok.', historico: [
    { autor: 'cliente', corpo: 'Die Qualität vom Polo Premium ist schlecht, ich will mein Geld zurück.', data: '2026-07-03T10:00:00.000Z' },
...
async function cliente(cls, { de, nome, corpo, ticketId, lojaId, comImagem, agora } = {}) {
  ...
  // o helper NAO mexe no corpo: o texto do cliente e exatamente o que o teste
  // declarou. Nada da saida simulada da IA pode voltar para a entrada.
...
test('cliente exigindo 100% em toda mensagem: uma etapa por resposta, 100% só com o dono', async () => {
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', resumo: 'material ruim, 100%!' }, { de: 'c1@web.de', nome: 'C1', corpo: 'Das Polo Premium ist schlecht. 100%!', lojaId: 'loja1' })
...
test('regeneração no modo novo: só a ação da fase, instrução não muda oferta, bloqueios valem', async () => {
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c3@web.de', nome: 'C3', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja1' })
...
test('edição manual no aprovar: fora da fase bloqueia; mudança de oferta exige confirmação e fica no histórico', async () => {
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c4@web.de', nome: 'C4', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja1' })
  await comEnvio('ok', () => aprovar(t)); t = await ticket(t.id)
...
test('auto-envio reconfere o rascunho: editado fora da fase ou com oferta mudada vai ao dono; intacto sai', async () => {
  ...
  try {
    ...
    let a = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c8@web.de', nome: 'C8', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: ha4min() })
    ...
    let b = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c9@web.de', nome: 'C9', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: ha4min() })
    await api(`/api/tickets/${b.id}/rascunho`, { texto: 'Hallo, wir melden uns bald.' })
    // rascunho intacto
    const c = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c10@web.de', nome: 'C10', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: ha4min() })
    // cliente holandês cujo rascunho foi trocado por um texto em alemão (mesma etapa, mesmos números)
    let d = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', idioma: 'nl' }, { de: 'c25@web.de', nome: 'C25', corpo: 'De kwaliteit van de Polo Premium is slecht, ik wil mijn geld terug.', lojaId: 'loja3', agora: ha4min() })
...
test('endereço incompleto: pede só o que falta e não encaminha o aceite', async () => {
  let t = await cliente({ intencao: 'pede_troca', motivo: 'errado' }, { de: 'c5@web.de', nome: 'C5', corpo: 'Das Polo Premium hat die falsche Farbe.', lojaId: 'loja1' })
...
test('imagem inadequada: imagem não é prova; troca só depois da validação do dono; recusa pede outra foto', async () => {
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'defeito' }, { de: 'c6@web.de', nome: 'C6', corpo: 'Das Polo Premium ist kaputt.', lojaId: 'loja1' })
...
test('aceite aprovado pelo dono: a confirmação nasce só do clique, com os números da opção aceita, e encerra o caso', async () => {
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c11@web.de', nome: 'C11', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja1' })
...
test('rota de foto recusada fora do fluxo de defeito (imagem em caso de qualidade não abre validação)', async () => {
  const t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c13@web.de', nome: 'C13', corpo: 'Das Polo Premium ist schlecht, siehe Foto.', lojaId: 'loja1', comImagem: true })
...
test('correção manual na Central guarda histórico de auditoria e não toca no motor', async () => {
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c12@web.de', nome: 'C12', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja1' })
  await comEnvio('ok', () => aprovar(t)); t = await ticket(t.id)
...
test('idioma do cliente manda: holandês com loja fixa em alemão, Bélgica (fr/nl), Áustria (de/en)', async () => {
  ...
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', idioma: 'nl' }, { de: 'c17@web.de', nome: 'C17', corpo: 'De kwaliteit van de Polo Premium valt tegen, ik wil mijn geld terug.', lojaId: 'loja1' })
  ...
  // francês da Bélgica
  t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', idioma: 'fr-BE' }, { de: 'c18@web.de', nome: 'C18', corpo: 'La qualité du Polo Premium est décevante, je souhaite un remboursement.', lojaId: 'loja1' })
  ...
  // holandês da Bélgica
  t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', idioma: 'nl-BE' }, { de: 'c19@web.de', nome: 'C19', corpo: 'De kwaliteit van de Polo Premium valt tegen, ik wil mijn geld terug.', lojaId: 'loja1' })
  assert.equal(an(t).idioma, 'nl'); assert.equal(an(t).idiomaOriginal, 'nl-BE'); assert.match(t.rascunho, /omruil/)
  // Áustria em alemão
  t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', idioma: 'de-AT' }, { de: 'c20@web.de', nome: 'C20', corpo: 'Die Qualität vom Polo Premium ist schlecht, ich will mein Geld zurück.', lojaId: 'loja1' })
  ...
  t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', idioma: 'en' }, { de: 'c21@web.de', nome: 'C21', corpo: 'The quality of the Polo Premium is poor, I would like a refund.', lojaId: 'loja1' })
...
test('endereço marcado pela IA como idiomaConfiavel: true não troca o idioma da conversa (o servidor decide)', async () => {
  let t = await cliente({ intencao: 'pede_troca', motivo: 'errado', idioma: 'nl' }, { de: 'c26@web.de', nome: 'C26', corpo: 'Ik heb de Polo Premium in de verkeerde kleur ontvangen, graag omruilen.', lojaId: 'loja1' })
...
test('conversa holandesa muda para alemão com "Polo ist zu klein" (curta, mas com significado)', async () => {
  let t = await cliente({ intencao: 'pede_troca', motivo: 'tamanho', idioma: 'nl' }, { de: 'c27@web.de', nome: 'C27', corpo: 'De maat van de Polo Premium klopt niet, ik wil graag omruilen.', lojaId: 'loja1' })
...
test('escritor no idioma errado: regenera uma vez; se insistir, vai ao dono sem enviar nem avançar — também na regeneração', async () => {
  ...
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', idioma: 'nl' }, { de: 'c23@web.de', nome: 'C23', corpo: 'De kwaliteit van de Polo Premium valt tegen, ik wil mijn geld terug.', lojaId: 'loja1' })
  ...
  t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', idioma: 'nl' }, { de: 'c24@web.de', nome: 'C24', corpo: 'De kwaliteit van de Polo Premium valt tegen, ik wil mijn geld terug.', lojaId: 'loja1' })
...
test('alternância antigo → novo → antigo: cada conversa fica no motor em que nasceu; não existe migração', async () => {
  ...
  let a = await cliente(null, { de: 'c31@web.de', nome: 'C31', corpo: 'Wo ist mein Paket mit dem Polo Premium?', lojaId: 'loja6' })
  ...
  let b = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c32@web.de', nome: 'C32', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja6' })
...
test('envio automático: só no novo, com confirmação, bloqueado no piloto; toda reativação do novo volta desligado', async () => {
  ...
  try {
    ...
    const t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c34@web.de', nome: 'C34', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja6' })
...
test('ponta a ponta — marcado como entregue: aguardar 2 dias (48 h reais do envio) → 20% → 35% → 100% com o dono; sem salto e sem reinício do relógio', async () => {
  ...
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'nao_recebido', situacaoEntrega: 'entregue_nao_recebido', resumo: 'consta entregue, nada chegou' }, { de: 'c41@web.de', nome: 'C41', corpo: 'Das Polo Premium: als zugestellt markiert, aber nichts angekommen. Geld zurück!', lojaId: 'loja1' })
  ...
  let u = await cliente({ intencao: 'pede_reembolso', motivo: 'nao_recebido', situacaoEntrega: 'entregue_nao_recebido' }, { de: 'c42@web.de', nome: 'C42', corpo: 'Das Polo Premium: zugestellt, aber nicht da.', lojaId: 'loja1' })
  ...
  let v = await cliente({ intencao: 'pede_reembolso', motivo: 'nao_recebido', situacaoEntrega: 'entregue_nao_recebido' }, { de: 'c43@web.de', nome: 'C43', corpo: 'Das Polo Premium: zugestellt, aber nicht da.', lojaId: 'loja1' })
...
test('rotas — modo novo SEM transição pendente: sem produto nada sai (manual, classificação falha, IA pausada); coleta manual sai; volta ao mesmo motivo; conta própria sempre; sem transição inventada', async () => {
  ...
  let b = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', produtos: [] }, { de: 'c62@web.de', nome: 'C62', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja1' })
...
test('cadência ponta a ponta (loja automática, envio liberado): 3 min na primeira resposta, 5 h depois, reinício pela mensagem mais recente, nada sai antes; clássico usa o seu atraso', async () => {
  ...
  try {
    ...
    let a = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c71@web.de', nome: 'C71', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: iso(Date.now() - 2 * MIN) })
    ...
    let b = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c72@web.de', nome: 'C72', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: iso(Date.now() - 5 * H - 4 * MIN) })
    ...
    let c = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c73@web.de', nome: 'C73', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: iso(Date.now() - 4 * MIN) })
    ...
    let d = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c76@web.de', nome: 'C76', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: iso(Date.now() - 5 * H - 10 * MIN) })
    ...
    const e = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c75@web.de', nome: 'C75', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3' })
    ...
    const k = await cliente(null, { de: 'c74@web.de', nome: 'C74', corpo: 'Wo ist mein Paket mit dem Polo Premium?', lojaId: 'loja2' })
...
test('separação definitiva antigo × novo: motor gravado no nascimento pela data real do primeiro e-mail × novoAtivadoEm, nunca recalculado', async () => {
  ...
  let a = await cliente(null, { de: 'c82@web.de', nome: 'C82', corpo: 'Wo ist mein Paket mit dem Polo Premium?', lojaId: 'loja6' })
  ...
  const antes = await cliente(null, { de: 'c83@web.de', nome: 'C83', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja6', agora: iso(ativadoEm - 60_000) }) // clássica: sem classificação do novo
  ...
  const depois = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c84@web.de', nome: 'C84', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja6', agora: iso(ativadoEm + 1000) })
  ...
  const entre = await cliente(null, { de: 'c85@web.de', nome: 'C85', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja6', agora: iso(novaAtivacao - 10) }) // clássica: sem classificação do novo
...
test('Base de Conhecimento é exclusiva do clássico: marcadores conflitantes entram no prompt e na resposta do clássico e NUNCA nos prompts do novo (1ª resposta, regeneração, aprovação, confirmação, autoenvio); falha da IA do novo vai ao dono sem fallback', async () => {
  ...
  const k2 = await cliente(null, { de: 'c92@web.de', nome: 'C92', corpo: 'Schlechte Qualität vom Polo Premium, Geld zurück!', lojaId: 'loja2' })
  ...
  promptsCapturados.length = 0
  let n = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade', resumo: 'qualidade ruim, quer 100%' }, { de: 'c93@web.de', nome: 'C93', corpo: 'Das Polo Premium ist schlecht. 100% zurück!', lojaId: 'loja1' })
  ...
  let a = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c94@web.de', nome: 'C94', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: new Date(Date.now() - 4 * 60_000).toISOString() })
  ...
  const f = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c95@web.de', nome: 'C95', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja1' })
...
const FASES_CONF = ['conf_troca', 'conf_reembolso', 'conf_cupom', 'conf_cancelamento']
...
test('aceite — aprovação humana (padrão): a IA negocia sem escalar; só o aceite vai ao dono; nada sai antes da aprovação; nenhuma linha automática no relatório', async () => {
  ...
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c101@web.de', nome: 'C101', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja1' })
  ...
  let u = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c102@web.de', nome: 'C102', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja1' })
...
test('aceite — aprovação concedida pelo dono (loja3, envio liberado): confirmação certa, autorizada e agendada; antes das 5 h não sai, depois sai; fase só muda com o envio real; sem linha automática', async () => {
  ...
  let a = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c103@web.de', nome: 'C103', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: iso(Date.now() - H5 - 4 * 60_000) })
  ...
  let b = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c104@web.de', nome: 'C104', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: iso(Date.now() - H5 - 10 * 60_000) })
...
test('aceite — conclusão automática (loja3 com a aprovação desligada): mesma negociação, aceite não vai ao dono, confirmação agendada para 5 h, nova mensagem reagenda, envio real cria exatamente UMA linha no relatório; idempotente', async () => {
  ...
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c105@web.de', nome: 'C105', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: iso(Date.now() - H5 - 4 * 60_000) })
  ...
  let u = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c106@web.de', nome: 'C106', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: iso(Date.now() - H5 - 20 * 60_000) })
...
test('aceite — sempre humano mesmo no automático: reembolso de 100%, cancelamento, defeito sem foto validada; e as falhas (texto fora da fase, percentual, valor, cupom, idioma, envio) nunca avançam fase nem criam relatório', async () => {
  ...
  let t = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c121@web.de', nome: 'C121', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
  ...
  let k = await cliente({ intencao: 'pede_cancelamento' }, { de: 'c122@web.de', nome: 'C122', corpo: 'Stornieren Sie bitte das Polo Premium!', lojaId: 'loja3', agora: antes() })
  ...
  let d = await cliente({ intencao: 'pede_troca', motivo: 'defeito' }, { de: 'c123@web.de', nome: 'C123', corpo: 'Das Polo Premium ist kaputt.', lojaId: 'loja3', comImagem: true, agora: antes() })
  ...
  for (const [de, n, texto, re] of casos) {
    let f = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de, nome: 'C' + n, corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
  ...
  let g = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c128@web.de', nome: 'C128', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
  ...
  let h = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c129@web.de', nome: 'C129', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
  ...
  let s4 = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c130@web.de', nome: 'C130', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja4' })
...
test('aceite — mudar a configuração não mexe em aceites pendentes (modo fotografado); nova ativação do novo volta para aprovação humana; Base conflitante não altera; clássico intacto', async () => {
  ...
  let a = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c131@web.de', nome: 'C131', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
  ...
  let b = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c132@web.de', nome: 'C132', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
...
test('mensagem nova durante a confirmação já autorizada (manual aprovada E automática): a confirmação antiga nunca sai; agradecimento reagenda a MESMA solução; voltar atrás cancela e vai ao dono', async () => {
  process.env.ATENDO_SMTP_FAKE = 'ok'
  try {
    const antes = () => iso(Date.now() - H5 - 20 * 60_000)
    let r = await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: true }); assert.equal(r.status, 200, r.erro)

    /* ---- A) MANUAL já aprovado pelo dono + agradecimento: mesma confirmação, 5 h reiniciadas, sem segunda aprovação ---- */
    let a = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c141@web.de', nome: 'C141', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
    a = await negociarAte(a, 'c141@web.de', 'reemb_40', { agora: antes })
    a = await cliente({ intencao: 'aceita', resumo: 'ok 40' }, { de: 'c141@web.de', corpo: 'Ok, 40%.', ticketId: a.id, agora: iso(Date.now() - H5 + 8_000) })
    assert.equal(a.status, 'humano'); assert.equal(an(a).conclusaoPendente.modo, 'manual')
    r = await api(`/api/tickets/${a.id}/novo/confirmar`); assert.equal(r.status, 200, r.erro); a = await ticket(a.id)
    const idA = an(a).conclusaoPendente.id; const aprovadoEm = an(a).conclusaoPendente.aprovadoEm
    assert.equal(an(a).conclusaoPendente.status, 'aguardando_cadencia'); assert.ok(aprovadoEm); assert.equal(an(a).transicaoPendente.para, 'conf_reembolso')
    assert.ok(Math.abs(a.enviaEm - (Date.now() + 8_000)) < 4000, 'agendada para as 5 h da última mensagem')
    // o cliente agradece antes do envio: a confirmação antiga NÃO sai; mesma solução, relógio reiniciado, autorização preservada
    a = await cliente({ intencao: 'informa', resumo: 'obrigado, quando sai?' }, { de: 'c141@web.de', corpo: 'Danke! Wann wird es bearbeitet?', ticketId: a.id, agora: iso(Date.now() - H5 + 20_000) })
    assert.equal(an(a).conclusaoPendente.id, idA, 'mesmo id do aceite'); assert.equal(an(a).conclusaoPendente.aprovadoEm, aprovadoEm, 'autorização do dono preservada')
    assert.equal(an(a).conclusaoPendente.status, 'aguardando_cadencia'); assert.equal(an(a).transicaoPendente.para, 'conf_reembolso', 'mesma confirmação, regerada')
    assert.match(a.rascunho, /40%/); assert.equal(a.status, 'aprovacao', 'não pede segunda aprovação')
    assert.ok(Math.abs(a.enviaEm - (Date.now() + 20_000)) < 5000, '5 h reiniciadas a partir da mensagem nova')
    assert.ok(an(a).historicoEtapas.some(h => h.evento === 'confirmacao_reagendada'))
    await esperar(6500); a = await ticket(a.id); assert.equal(an(a).etapa, 'reemb_40', 'a confirmação antiga não saiu antes da hora nova')
    // chegada a hora reagendada: sai UMA única confirmação, a mesma solução aprovada
    a = await enviadoAte(a.id, 'enviado', 40); assert.equal(an(a).etapa, 'conf_reembolso'); assert.equal(an(a).conclusaoPendente.id, idA); assert.equal(an(a).historicoEtapas.filter(h => h.para === 'conf_reembolso').length, 1)
    assert.equal(a.relatorioAuto, undefined, 'modo manual: relatório continua do dono')

    /* ---- B) MANUAL já aprovado + cliente VOLTA ATRÁS: confirmação cancelada, caso humano, sem próxima oferta automática ---- */
    let b = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c142@web.de', nome: 'C142', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
    b = await negociarAte(b, 'c142@web.de', 'reemb_40', { agora: antes })
    b = await cliente({ intencao: 'aceita', resumo: 'ok 40' }, { de: 'c142@web.de', corpo: 'Ok, 40%.', ticketId: b.id, agora: iso(Date.now() - H5 + 30_000) })
    r = await api(`/api/tickets/${b.id}/novo/confirmar`); assert.equal(r.status, 200, r.erro); b = await ticket(b.id)
    const idB = an(b).conclusaoPendente.id; assert.ok(b.enviaEm)
    b = await cliente({ intencao: 'recusa', resumo: 'mudei de ideia, quero 50%' }, { de: 'c142@web.de', corpo: 'Doch nicht! Ich will 50%.', ticketId: b.id, agora: iso(Date.now() - H5 + 40_000) })
    assert.equal(b.status, 'humano', 'vai ao dono'); assert.match(b.motivoEscalada, /voltou atrás/)
    assert.equal(an(b).conclusaoPendente.id, idB); assert.equal(an(b).conclusaoPendente.status, 'cancelada')
    assert.equal(an(b).transicaoPendente, null, 'transição de confirmação removida'); assert.equal(b.rascunho, undefined, 'rascunho removido'); assert.equal(b.enviaEm, undefined, 'agendamento removido')
    assert.equal(an(b).etapa, 'reemb_40', 'não avançou para o 50% sozinho'); assert.equal(b.relatorioAuto, undefined); assert.equal(b.relatorioDia, undefined)
    await esperar(6500); b = await ticket(b.id); assert.equal(an(b).etapa, 'reemb_40'); assert.equal(b.status, 'humano'); assert.equal(an(b).historicoEtapas.filter(h => h.para === 'conf_reembolso').length, 0, 'nenhuma confirmação antiga saiu')

    /* ---- C) AUTOMÁTICO + agradecimento: mesma conclusão e relógio reiniciado ---- */
    r = await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: false, confirmar: true }); assert.equal(r.status, 200, r.erro)
    let c = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c143@web.de', nome: 'C143', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
    c = await negociarAte(c, 'c143@web.de', 'reemb_40', { agora: antes })
    c = await cliente({ intencao: 'aceita', resumo: 'ok 40' }, { de: 'c143@web.de', corpo: 'Ok, 40%.', ticketId: c.id, agora: iso(Date.now() - H5 + 8_000) })
    const idC = an(c).conclusaoPendente.id; assert.equal(an(c).conclusaoPendente.modo, 'automatico'); assert.equal(c.status, 'aprovacao')
    c = await cliente({ intencao: 'informa', resumo: 'obrigado' }, { de: 'c143@web.de', corpo: 'Danke!', ticketId: c.id, agora: iso(Date.now() - H5 + 20_000) })
    assert.equal(an(c).conclusaoPendente.id, idC); assert.equal(an(c).conclusaoPendente.status, 'aguardando_cadencia'); assert.equal(an(c).transicaoPendente.para, 'conf_reembolso')
    assert.ok(Math.abs(c.enviaEm - (Date.now() + 20_000)) < 5000, 'relógio reiniciado')
    await esperar(6500); c = await ticket(c.id); assert.equal(an(c).etapa, 'reemb_40', 'nada saiu antes da hora')
    c = await enviadoAte(c.id, 'enviado', 40); assert.equal(an(c).etapa, 'conf_reembolso'); assert.equal(an(c).conclusaoPendente.id, idC); assert.equal(an(c).historicoEtapas.filter(h => h.para === 'conf_reembolso').length, 1); assert.equal(c.relatorioAuto.eventoId, idC, 'uma linha, do mesmo aceite')

    /* ---- D) AUTOMÁTICO + cliente volta atrás: cancelado e humano ---- */
    let d = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c144@web.de', nome: 'C144', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
    d = await negociarAte(d, 'c144@web.de', 'reemb_40', { agora: antes })
    d = await cliente({ intencao: 'aceita', resumo: 'ok 40' }, { de: 'c144@web.de', corpo: 'Ok, 40%.', ticketId: d.id, agora: iso(Date.now() - H5 + 30_000) })
    const idD = an(d).conclusaoPendente.id; assert.ok(d.enviaEm)
    d = await cliente({ intencao: 'pede_troca', resumo: 'mudei de ideia: quero troca' }, { de: 'c144@web.de', corpo: 'Lieber einen Umtausch!', ticketId: d.id, agora: iso(Date.now() - H5 + 40_000) })
    assert.equal(d.status, 'humano'); assert.match(d.motivoEscalada, /voltou atrás/); assert.equal(an(d).conclusaoPendente.id, idD); assert.equal(an(d).conclusaoPendente.status, 'cancelada')
    assert.equal(an(d).transicaoPendente, null); assert.equal(d.enviaEm, undefined); assert.equal(d.rascunho, undefined); assert.equal(an(d).etapa, 'reemb_40'); assert.equal(d.relatorioAuto, undefined)
    await esperar(6500)
    for (const x of [b, d]) { const y = await ticket(x.id); assert.equal(y.status, 'humano'); assert.equal(y.relatorioAuto, undefined); assert.equal(an(y).historicoEtapas.filter(h => FASES_CONF.includes(h.para)).length, 0, 'nenhuma confirmação antiga saiu em ' + y.id) }
    await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: true })
  } finally { delete process.env.ATENDO_SMTP_FAKE }
})

test('configuração incompatível: desligar o envio automático da loja ou a automação geral volta a exigir aprovação, interrompe as conclusões automáticas pendentes (sem confirmação e sem relatório) e registra a auditoria', async () => {
  process.env.ATENDO_SMTP_FAKE = 'ok'
  try {
    const antes = () => iso(Date.now() - H5 - 20 * 60_000)
    /* ---- A) desligar o ENVIO AUTOMÁTICO da loja ---- */
    let r = await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: false, confirmar: true }); assert.equal(r.status, 200, r.erro)
    let a = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c151@web.de', nome: 'C151', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
    a = await negociarAte(a, 'c151@web.de', 'reemb_40', { agora: antes })
    a = await cliente({ intencao: 'aceita', resumo: 'ok 40' }, { de: 'c151@web.de', corpo: 'Ok, 40%.', ticketId: a.id, agora: iso(Date.now() - H5 + 2 * 60_000) })
    const idA = an(a).conclusaoPendente.id; assert.equal(an(a).conclusaoPendente.modo, 'automatico'); assert.equal(a.status, 'aprovacao'); assert.ok(a.enviaEm)
    // ação de segurança: desligar o envio automático é permitido e neutraliza a conclusão automática
    r = await api('/api/lojas', { id: 'loja3', novoEnvioAutomatico: false }); assert.equal(r.status, 200, r.erro)
    const l3 = r.state.lojas.find(l => l.id === 'loja3')
    assert.equal(l3.novoEnvioAutomatico, false); assert.equal(l3.exigirAprovacaoAceiteNovo, true, 'aprovação volta a ser obrigatória')
    const aud = l3.aceiteHistorico.at(-1); assert.equal(aud.de, false); assert.equal(aud.para, true); assert.match(aud.motivo, /envio automático da loja foi desligado/)
    a = await ticket(a.id)
    assert.equal(a.status, 'humano', 'aceite pendente aparece para o dono'); assert.match(a.motivoEscalada, /Conclusão automática interrompida porque o envio automático da loja foi desligado\./)
    assert.equal(an(a).conclusaoPendente.id, idA); assert.equal(an(a).conclusaoPendente.status, 'interrompida'); assert.equal(an(a).conclusaoPendente.valor, 40, 'dados da solução preservados')
    assert.equal(a.enviaEm, undefined, 'sem agendamento'); assert.equal(an(a).etapa, 'reemb_40'); assert.equal(a.relatorioAuto, undefined); assert.equal(a.relatorioDia, undefined)
    await esperar(6500); a = await ticket(a.id); assert.equal(an(a).etapa, 'reemb_40', 'nenhuma confirmação saiu'); assert.equal(a.relatorioAuto, undefined)
    // aceites NOVOS passam a ser manuais
    await api('/api/lojas', { id: 'loja3', novoEnvioAutomatico: true, confirmar: true })
    let novo = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c152@web.de', nome: 'C152', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
    novo = await negociarAte(novo, 'c152@web.de', 'qual_cupom_35', { agora: antes })
    novo = await cliente({ intencao: 'aceita', resumo: 'ok' }, { de: 'c152@web.de', corpo: 'Ok.', ticketId: novo.id, agora: antes() })
    assert.equal(novo.status, 'humano'); assert.equal(an(novo).conclusaoPendente.modo, 'manual', 'novos aceites exigem aprovação')

    /* ---- B) desligar a AUTOMAÇÃO GERAL ---- */
    r = await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: false, confirmar: true }); assert.equal(r.status, 200, r.erro)
    let b = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c153@web.de', nome: 'C153', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
    b = await negociarAte(b, 'c153@web.de', 'reemb_40', { agora: antes })
    b = await cliente({ intencao: 'aceita', resumo: 'ok 40' }, { de: 'c153@web.de', corpo: 'Ok, 40%.', ticketId: b.id, agora: iso(Date.now() - H5 + 2 * 60_000) })
    assert.equal(an(b).conclusaoPendente.modo, 'automatico'); assert.ok(b.enviaEm)
    r = await api('/api/config', { automacaoAtiva: false }); assert.equal(r.status, 200, r.erro)
    const l3b = r.state.lojas.find(l => l.id === 'loja3'); assert.equal(l3b.exigirAprovacaoAceiteNovo, true, 'todas as lojas voltam a exigir aprovação')
    assert.match(l3b.aceiteHistorico.at(-1).motivo, /automação geral foi desligada/)
    b = await ticket(b.id)
    assert.equal(b.status, 'humano', 'não fica escondido em aprovação sem responsável'); assert.match(b.motivoEscalada, /automação geral foi desligada/)
    assert.equal(an(b).conclusaoPendente.status, 'interrompida'); assert.equal(b.enviaEm, undefined); assert.equal(b.relatorioAuto, undefined)
    // religar a automação global NÃO retoma as conclusões interrompidas
    r = await api('/api/config', { automacaoAtiva: true }); assert.equal(r.status, 200, r.erro)
    assert.equal(r.state.lojas.find(l => l.id === 'loja3').exigirAprovacaoAceiteNovo, true, 'a aprovação continua obrigatória')
    await esperar(6500); b = await ticket(b.id)
    assert.equal(b.status, 'humano'); assert.equal(an(b).conclusaoPendente.status, 'interrompida'); assert.equal(an(b).etapa, 'reemb_40'); assert.equal(b.relatorioAuto, undefined); assert.equal(b.enviaEm, undefined)
    // e o dono ainda pode aprovar normalmente: a solução aceita foi preservada
    r = await api(`/api/tickets/${b.id}/novo/confirmar`); assert.equal(r.status, 200, r.erro); b = await ticket(b.id)
    assert.equal(an(b).transicaoPendente.para, 'conf_reembolso'); assert.match(b.rascunho, /40%/)
  } finally { delete process.env.ATENDO_SMTP_FAKE }
})

test('conclusão automática interrompida vira MANUAL de verdade: o dono aprova, a confirmação sai uma vez e relatorioAuto continua ausente; troca aguardando endereço não é quebrada', async () => {
  process.env.ATENDO_SMTP_FAKE = 'ok'
  try {
    const antes = () => iso(Date.now() - H5 - 20 * 60_000)
    let r = await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: false, confirmar: true }); assert.equal(r.status, 200, r.erro)

    /* ---- A) reembolso aceito, automação desligada, DONO aprova: uma confirmação e NENHUM relatorioAuto ---- */
    let a = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de: 'c161@web.de', nome: 'C161', corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3', agora: antes() })
    a = await negociarAte(a, 'c161@web.de', 'reemb_40', { agora: antes })
    a = await cliente({ intencao: 'aceita', resumo: 'ok 40' }, { de: 'c161@web.de', corpo: 'Ok, 40%.', ticketId: a.id, agora: iso(Date.now() - H5 + 2 * 60_000) })
    const idA = an(a).conclusaoPendente.id
    assert.equal(an(a).conclusaoPendente.modo, 'automatico'); assert.equal(an(a).transicaoPendente.para, 'conf_reembolso'); assert.ok(a.enviaEm)
    r = await api('/api/lojas', { id: 'loja3', novoEnvioAutomatico: false }); assert.equal(r.status, 200, r.erro)
    a = await ticket(a.id)
    const cpA = an(a).conclusaoPendente
    assert.equal(cpA.id, idA, 'mesmo id do aceite'); assert.equal(cpA.modo, 'manual', 'a continuação virou MANUAL de verdade')
    assert.equal(cpA.modoOriginal, 'automatico', 'auditoria: era automática'); assert.equal(cpA.relatorioAutomaticoProibido, true)
    assert.equal(cpA.status, 'interrompida'); assert.equal(cpA.valor, 40); assert.deepEqual(cpA.produtos, ['Polo Premium (Schwarz / L)'])
    assert.equal(an(a).transicaoPendente, null, 'rascunho/transição antigos invalidados'); assert.equal(a.rascunho, undefined); assert.equal(a.enviaEm, undefined)
    assert.equal(a.status, 'humano'); assert.equal(an(a).acaoAceita, 'reemb_40')
    await esperar(6500); a = await ticket(a.id); assert.equal(an(a).etapa, 'reemb_40', 'nenhuma confirmação antiga saiu')
    // o dono aprova: a confirmação é REGERADA e revalidada; sai uma vez; sem relatório automático
    r = await api(`/api/tickets/${a.id}/novo/confirmar`); assert.equal(r.status, 200, r.erro); a = await ticket(a.id)
    assert.equal(an(a).transicaoPendente.para, 'conf_reembolso'); assert.match(a.rascunho, /40%/); assert.match(a.rascunho, /40,00 €/)
    r = await comEnvio('ok', () => aprovar(a)); assert.equal(r.status, 200, r.erro); a = await ticket(a.id)
    assert.equal(an(a).etapa, 'conf_reembolso'); assert.equal(an(a).historicoEtapas.filter(h => h.para === 'conf_reembolso').length, 1, 'uma única confirmação')
    assert.equal(an(a).conclusaoPendente.status, 'concluida'); assert.equal(an(a).conclusaoPendente.id, idA)
    assert.equal(a.relatorioAuto, undefined, 'conclusão convertida para manual NUNCA cria relatorioAuto'); assert.equal(a.relatorioDia, undefined, 'o relatório continua do dono')

    /* ---- B) troca aceita AGUARDANDO ENDEREÇO quando a automação é desligada: a coleta continua ---- */
    r = await api('/api/lojas', { id: 'loja3', novoEnvioAutomatico: true, confirmar: true }); assert.equal(r.status, 200, r.erro)
    r = await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: false, confirmar: true }); assert.equal(r.status, 200, r.erro)
    let b = await cliente({ intencao: 'pede_reembolso', motivo: 'tamanho', ajustes: [{ produto: 'Polo Premium', ajuste: 'pequeno' }] }, { de: 'c162@web.de', nome: 'C162', corpo: 'Das Polo Premium ist zu klein.', lojaId: 'loja3', agora: antes() })
    if (an(b).transicaoPendente.para === 'tam_ajuste') { const r0 = await comEnvio('ok', () => aprovar(b)); assert.equal(r0.status, 200, r0.erro); b = await cliente({ intencao: 'informa', ajustes: [{ produto: 'Polo Premium', ajuste: 'pequeno' }], resumo: 'ajuste' }, { de: 'c162@web.de', corpo: 'zu klein', ticketId: b.id, agora: antes() }) }
    b = await negociarAte(b, 'c162@web.de', 'tam_troca', { agora: antes })
    b = await cliente({ intencao: 'aceita', resumo: 'ok troca' }, { de: 'c162@web.de', corpo: 'Ja, Umtausch!', ticketId: b.id, agora: antes() })
    const idB = an(b).conclusaoPendente.id
    assert.equal(an(b).conclusaoPendente.status, 'aguardando_dados'); assert.equal(an(b).transicaoPendente.para, 'endereco', 'pergunta do endereço pendente')
    // desliga a automação global bem aqui: a pergunta do endereço ainda NÃO saiu
    r = await api('/api/config', { automacaoAtiva: false }); assert.equal(r.status, 200, r.erro)
    b = await ticket(b.id)
    assert.equal(an(b).conclusaoPendente.id, idB); assert.equal(an(b).conclusaoPendente.modo, 'manual'); assert.equal(an(b).conclusaoPendente.relatorioAutomaticoProibido, true)
    assert.equal(an(b).conclusaoPendente.status, 'aguardando_dados', 'a coleta de endereço foi preservada')
    assert.equal(an(b).transicaoPendente.para, 'endereco', 'a pergunta continua pendente'); assert.equal(b.status, 'aprovacao', 'fica em Aprovações para envio manual')
    assert.notEqual(an(b).aguardando, 'humano', 'não fica preso em aguardando: humano')
    // envio manual da pergunta e resposta do cliente: o endereço é validado normalmente
    r = await api('/api/config', { automacaoAtiva: true })
    r = await comEnvio('ok', () => aprovar(b)); assert.equal(r.status, 200, r.erro); b = await ticket(b.id); assert.equal(an(b).etapa, 'endereco')
    b = await cliente({ intencao: 'informa', endereco: 'Hauptstr. 5', resumo: 'rua' }, { de: 'c162@web.de', corpo: 'Hauptstr. 5', ticketId: b.id, agora: antes() })
    assert.equal(an(b).transicaoPendente.para, 'endereco', 'pede só o que falta'); assert.ok(an(b).transicaoPendente.faltando.length)
    r = await comEnvio('ok', () => aprovar(b)); assert.equal(r.status, 200, r.erro)
    b = await cliente({ intencao: 'informa', endereco: '10115 Berlin', resumo: 'cep e cidade' }, { de: 'c162@web.de', corpo: '10115 Berlin', ticketId: b.id, agora: antes() })
    // dados completos: vai ao DONO (a conclusão é manual agora), nunca confirmação automática
    assert.equal(b.status, 'humano', 'com os dados completos o caso vai ao dono'); assert.equal(an(b).conclusaoPendente.id, idB)
    assert.match(an(b).conclusaoPendente.endereco ?? an(b).enderecoConfirmado, /Hauptstr\. 5/)
    assert.notEqual(an(b).transicaoPendente?.para, 'conf_troca', 'nenhuma confirmação automática')
    r = await api(`/api/tickets/${b.id}/novo/confirmar`); assert.equal(r.status, 200, r.erro); b = await ticket(b.id)
    assert.equal(an(b).transicaoPendente.para, 'conf_troca'); assert.match(b.rascunho, /Hauptstr/)
    r = await comEnvio('ok', () => aprovar(b)); assert.equal(r.status, 200, r.erro); b = await ticket(b.id)
    assert.equal(an(b).etapa, 'conf_troca'); assert.equal(b.relatorioAuto, undefined, 'sem relatório automático'); assert.equal(an(b).conclusaoPendente.status, 'concluida')

  } finally { delete process.env.ATENDO_SMTP_FAKE }
})

test('proteção IMEDIATA em toda alteração de configuração: a própria resposta da rota já mostra a aprovação obrigatória e a conclusão convertida para manual (cupom, prazo, modo clássico e remoção do e-mail), e restaurar não religa sozinho', async () => {
  process.env.ATENDO_SMTP_FAKE = 'ok'
  try {
    const antes = () => iso(Date.now() - H5 - 20 * 60_000)
    const CUPONS = { 15: 'DANKE15', 25: 'SORRY25', 30: 'BACK30', 35: 'KEEP35', 40: 'WAIT40' }
    // prepara uma conclusão AUTOMÁTICA pendente na loja3 e devolve o ticket
    const comAceiteAutomatico = async (de, nome) => {
      let r = await api('/api/lojas', { id: 'loja3', novoEnvioAutomatico: true, confirmar: true }); assert.equal(r.status, 200, r.erro)
      r = await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: false, confirmar: true }); assert.equal(r.status, 200, r.erro)
      // a conversa nasce AGORA (a reativação do novo grava uma data de ativação nova: e-mail anterior a ela nasceria clássico)
      let x = await cliente({ intencao: 'pede_reembolso', motivo: 'qualidade' }, { de, nome, corpo: 'Das Polo Premium ist schlecht.', lojaId: 'loja3' })
      assert.equal(x.motorAtendimento, 'novo', de)
      x = await negociarAte(x, de, 'reemb_40')
      x = await cliente({ intencao: 'aceita', resumo: 'ok 40' }, { de, corpo: 'Ok, 40%.', ticketId: x.id, agora: iso(Date.now() - H5 + 2 * 60_000) })
      assert.equal(an(x).conclusaoPendente.modo, 'automatico', de); assert.ok(x.enviaEm, de)
      return x
    }
    // a RESPOSTA da rota (sem esperar, sem polling) já tem de mostrar tudo protegido
    const protegido = (resposta, ticketId, idAceite, reMotivo) => {
      const loja = resposta.state.lojas.find(l => l.id === 'loja3')
      assert.equal(loja.exigirAprovacaoAceiteNovo, true, 'resposta já mostra a aprovação obrigatória')
      const aud = loja.aceiteHistorico.at(-1)
      assert.equal(aud.de, false); assert.equal(aud.para, true); assert.match(aud.motivo, reMotivo, 'auditoria com o motivo exato')
      const tk = resposta.state.tickets.find(x => x.id === ticketId)
      const cp = tk.atendimentoNovo.conclusaoPendente
      assert.equal(cp.id, idAceite, 'mesmo aceite'); assert.equal(cp.modo, 'manual', 'resposta já mostra a conclusão manual')
      assert.equal(cp.modoOriginal, 'automatico'); assert.equal(cp.relatorioAutomaticoProibido, true)
      assert.equal(tk.enviaEm, undefined, 'agendamento removido na própria resposta')
      assert.equal(tk.relatorioAuto, undefined, 'nenhum relatório automático')
      return tk
    }

    /* ---- A) remover um cupom obrigatório ---- */
    let a = await comAceiteAutomatico('c171@web.de', 'C171')
    const idA = an(a).conclusaoPendente.id
    let r = await api('/api/lojas', { id: 'loja3', cupons: { ...CUPONS, 35: '' } }) // KEEP35 removido
    assert.equal(r.status, 200, r.erro)
    const tkA = protegido(r, a.id, idA, /cupons obrigatórios/)
    assert.equal(tkA.atendimentoNovo.etapa, 'reemb_40', 'a fase não avançou')
    await esperar(6500); a = await ticket(a.id)
    assert.equal(an(a).etapa, 'reemb_40', 'nenhum envio'); assert.equal(a.relatorioAuto, undefined, 'nenhum relatório automático')
    // restaurar o cupom NÃO religa a conclusão automática
    r = await api('/api/lojas', { id: 'loja3', cupons: CUPONS }); assert.equal(r.status, 200, r.erro)
    assert.equal(r.state.lojas.find(l => l.id === 'loja3').exigirAprovacaoAceiteNovo, true, 'recolocar o cupom não desliga a aprovação')
    assert.equal(r.state.tickets.find(x => x.id === a.id).atendimentoNovo.conclusaoPendente.modo, 'manual', 'a conclusão continua manual')

    /* ---- B) prazo mínimo zero ---- */
    let b = await comAceiteAutomatico('c172@web.de', 'C172')
    const idB = an(b).conclusaoPendente.id
    r = await api('/api/lojas', { id: 'loja3', prazoEntrega: { min: 0, max: 12, processamento: 3 } }); assert.equal(r.status, 200, r.erro)
    const tkB = protegido(r, b.id, idB, /prazo de entrega/)
    assert.equal(tkB.atendimentoNovo.etapa, 'reemb_40')
    await esperar(6500); b = await ticket(b.id); assert.equal(an(b).etapa, 'reemb_40'); assert.equal(b.relatorioAuto, undefined)
    // restaurar o prazo NÃO religa
    r = await api('/api/lojas', { id: 'loja3', prazoEntrega: { min: 5, max: 12, processamento: 3 } }); assert.equal(r.status, 200, r.erro)
    assert.equal(r.state.lojas.find(l => l.id === 'loja3').exigirAprovacaoAceiteNovo, true)

    /* ---- C) loja volta ao clássico: conversas continuam no motor em que nasceram ---- */
    let c = await comAceiteAutomatico('c173@web.de', 'C173')
    const idC = an(c).conclusaoPendente.id
    r = await api('/api/lojas/loja3/modo', { modo: 'classico', confirmar: true }); assert.equal(r.status, 200, r.erro)
    const tkC = protegido(r, c.id, idC, /voltou ao atendimento clássico/)
    assert.equal(tkC.motorAtendimento, 'novo', 'a conversa continua no motor em que nasceu')
    assert.equal(tkC.atendimentoNovo.etapa, 'reemb_40')
    for (const x of r.state.tickets.filter(x => x.motorAtendimento === 'novo' && (x.lojaId ?? 'loja1') === 'loja3')) assert.equal(x.motorAtendimento, 'novo', x.id + ': motor preservado')
    await esperar(6500); c = await ticket(c.id); assert.equal(an(c).etapa, 'reemb_40'); assert.equal(c.relatorioAuto, undefined)
    // voltar ao novo NÃO religa a conclusão automática (a ativação zera para aprovação obrigatória)
    r = await api('/api/lojas/loja3/modo', { modo: 'novo', confirmar: true }); assert.equal(r.status, 200, r.erro)
    assert.equal(r.state.lojas.find(l => l.id === 'loja3').exigirAprovacaoAceiteNovo, true, 'reativar o novo não desliga a aprovação')
    assert.equal(r.state.tickets.find(x => x.id === c.id).atendimentoNovo.conclusaoPendente.modo, 'manual')

    /* ---- D) remover a conta de e-mail: a resposta já vem protegida, SEM espera nem polling ---- */
    let d = await comAceiteAutomatico('c174@web.de', 'C174')
    const idD = an(d).conclusaoPendente.id
    r = await api('/api/lojas/loja3/email', null, 'DELETE'); assert.equal(r.status, 200, r.erro)
    const tkD = protegido(r, d.id, idD, /conta de e-mail da loja foi removida/) // nenhuma espera antes desta conferência
    assert.equal(tkD.status, 'humano'); assert.equal(tkD.atendimentoNovo.transicaoPendente, null, 'rascunho antigo invalidado')
    await esperar(6500); d = await ticket(d.id); assert.equal(an(d).etapa, 'reemb_40'); assert.equal(d.relatorioAuto, undefined)

    /* ---- E) o agendador continua sendo a última barreira ---- */
    // conclusão automática pendente com o agendamento vencido, mas os pré-requisitos já caíram (loja sem e-mail)
    const st = (await api('/api/state', null, 'GET')).state
    assert.equal(st.lojas.find(l => l.id === 'loja3').exigirAprovacaoAceiteNovo, true)
    const semEmail = st.tickets.find(x => x.id === d.id)
    assert.equal(semEmail.enviaEm, undefined); assert.equal(semEmail.relatorioAuto, undefined)
    // sem caixa própria a loja nem pode voltar a desligar a aprovação: a proteção se mantém
    r = await api('/api/lojas', { id: 'loja3', exigirAprovacaoAceiteNovo: false, confirmar: true })
    assert.equal(r.status, 400, 'sem e-mail não dá para religar a conclusão automática'); assert.match(r.erro, /Falta/)
    const l3 = (await api('/api/state', null, 'GET')).state.lojas.find(l => l.id === 'loja3')
    assert.equal(l3.exigirAprovacaoAceiteNovo, true, 'a aprovação continua obrigatória')
  } finally { delete process.env.ATENDO_SMTP_FAKE }
})

test('loja clássica não passa pelo motor novo', async () => {
  const t = await cliente(null, { de: 'c7@web.de', nome: 'C7', corpo: 'Wo ist mein Paket mit dem Polo Premium?', lojaId: 'loja2' })
