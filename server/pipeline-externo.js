...
const TOM_JORNADA = { entrada: 'var(--purple)', tamanho: 'var(--azul)', qualidade: 'var(--roxo)', defeito_errado: 'var(--red)', nao_recebido: 'var(--amber)', cancelamento: 'var(--green)' }
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .notice i{width:8px;height:8px;flex:0 0 auto;margin-top:6px;border-radius:50%;background:var(--amber);box-shadow:0 0 0 5px rgba(202,152,73,.16)}
  ...
  .kpi::after{content:"";position:absolute;width:95px;height:70px;right:-25px;bottom:-35px;border-radius:50%;background:var(--glow,rgba(82,156,202,.12));filter:blur(14px)}
  ...
  .tipo-confirmacao .stage-index{background:var(--ok-bg);color:var(--green);border-color:var(--ok-border)}
  ...
  .product-chip{display:inline-flex;padding:4px 7px;border-radius:8px;background:var(--roxo-bg);border:1px solid var(--roxo-border);color:var(--roxo);font-size:.68rem}
  ...
  .badge.manual{color:var(--green);background:var(--ok-bg);border-color:var(--ok-border)}
  .badge.warn{color:var(--amber);background:var(--warn-bg);border-color:var(--warn-border)}
  .badge.local{color:var(--roxo);background:var(--roxo-bg);border-color:var(--roxo-border)}
  ...
  .backdrop{position:fixed;inset:0;background:rgba(0,0,0,.6);backdrop-filter:blur(3px);z-index:20;opacity:0;pointer-events:none;transition:.2s}
  .backdrop.open{opacity:1;pointer-events:auto}
  .drawer{position:fixed;z-index:21;top:0;right:0;width:min(720px,92vw);height:100vh;display:flex;flex-direction:column;background:var(--panel);border-left:1px solid var(--border);box-shadow:-28px 0 80px rgba(0,0,0,.5);transform:translateX(103%);transition:.24s ease}
...
<div class="app">
  ...
  <main class="main">
    ...
    <section class="kpis" aria-label="Funil geral de pedidos e casos"><article class="kpi"><span>Pedidos totais</span><div class="kpi-main"><strong id="kpiTotalOrders" data-num>0</strong><em id="kpiTotalOrdersPct" data-num>100%</em></div><small>base de pedidos da loja nos filtros</small></article><article class="kpi"><span>Pedidos com ticket</span><div class="kpi-main"><strong id="kpiTicketOrders" data-num>0</strong><em id="kpiTicketPct" data-num>—</em></div><small>percentual sobre pedidos totais</small></article><article class="kpi" style="--glow:rgba(223,84,82,.14)"><span>Pedidos sobre reembolso</span><div class="kpi-main"><strong style="color:var(--red)" id="kpiRefundOrders" data-num>0</strong><em id="kpiRefundPct" data-num>—</em></div><small>percentual dos tickets abertos</small></article><article class="kpi" style="--glow:rgba(202,152,73,.14)"><span>Valor dos pedidos totais</span><div class="kpi-main"><strong class="kpi-valor" style="color:var(--amber)" id="kpiTotalOrderValue" data-num>—</strong><em id="kpiTotalValuePct" data-num>100%</em></div><small>soma de todos os pedidos da loja</small></article><article class="kpi" style="--glow:rgba(82,158,114,.14)"><span>Valor dos pedidos reembolsados</span><div class="kpi-main"><strong class="kpi-valor" style="color:var(--green)" id="kpiRefundedValue" data-num>—</strong><em id="kpiRefundedValuePct" data-num>0%</em></div><small id="kpiRefundedValueNote">reembolsado de fato sobre o valor total</small></article></section>
    ...
    <section class="flow-view" id="flowView"><div class="flow-shell"><article class="panel"><div class="panel-head"><div><h2 id="journeyTitle">${escapar(catalogo.jornadas[ordem[0]] ?? ordem[0])}</h2><p id="journeyDescription">${escapar(DESCRICAO_JORNADA[ordem[0]] ?? '')}</p></div><div class="panel-total"><strong id="journeyOrders" data-num>0 pedidos</strong><span id="journeyValue" data-num>— em valor</span></div></div>${fluxos}</article><aside class="panel insight-panel"><div class="panel-head"><div><h2>Leitura da jornada</h2><p>Resultado dos pedidos filtrados</p></div></div><div class="insight-body"><div class="health"><div class="ring" id="retentionRing" style="--progress:0%"><div class="ring-label"><strong id="retentionPct" data-num>0%</strong><span>antes do final</span></div></div></div><div><div class="insight-list"><div class="insight-row"><span>Pedidos da jornada</span><strong id="insightTotal" data-num>0</strong></div><div class="insight-row"><span>Pararam antes do fim</span><strong id="insightStopped" data-num>0</strong></div><div class="insight-row"><span>Chegaram ao final</span><strong id="insightFinal" data-num>0</strong></div><div class="insight-row"><span>Valor da jornada</span><strong id="insightValue" data-num>—</strong></div></div><div class="legend"><div><i style="--dot:var(--purple)"></i>Fase clicável — abra para ver os pedidos</div><div><i style="--dot:var(--amber)"></i>Valor soma apenas pedidos com preço localizado, por moeda</div><div><i style="--dot:var(--roxo)"></i>Ajustes manuais ficam neste navegador</div></div></div></div></aside></div></section>
