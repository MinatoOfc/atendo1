...
const SCHEMA = {
  ...
  required: ['categoria', 'idioma', 'resposta', 'confianca', 'escalar_humano', 'aprova_reembolso', 'confirma_troca', 'encerrar', 'motivo', 'spam', 'situacao', 'resolucao'],
  properties: {
    ...
    encerrar: { type: 'boolean', description: 'true quando a mensagem NÃO pede resposta nem ação: agradecimento, "tudo certo", "ok, obrigado", confirmação final de assunto já resolvido. A conversa é encerrada sem enviar nada e sem envolver o lojista. Havendo QUALQUER pergunta ou pendência, false.' },
...
export function montarSystem(state, ticket) {
  ...
  return [
    ...
    `- PROIBIDO, em QUALQUER etapa (inclusive dentro do fluxo de devolução), escrever frases que deem reembolso, cancelamento ou troca como CERTOS: "confirmo o reembolso", "o valor será estornado", "o dinheiro chegará em X dias", "o cancelamento está concluído", "a troca está confirmada" — em nenhum idioma. Você OFERECE e PERGUNTA; quem confirma é o lojista. O sistema detecta essas frases e bloqueia o envio, então escrevê-las só atrasa o cliente.`,
    `- Também PROIBIDO prometer que a troca já vai acontecer: "enviaremos as novas peças", "faremos a troca imediatamente", "sua troca já está em andamento", "vamos começar a produção", prazo de entrega ou código de rastreio da peça nova. Quando o cliente aceita a troca e diz o tamanho/cor, o seu trabalho ACABOU: escale (confirma_troca=true) com resposta VAZIA e deixe o lojista despachar e avisar. Prometer envio no lugar dele é o pior erro que você pode cometer aqui.`,
    ...
    ``,
    `NUNCA repita uma resposta igual ou parecida com a sua última mensagem do histórico. Se o cliente mandou de novo o mesmo pedido, é porque a sua resposta anterior NÃO resolveu — releia o que ele pediu e responda ESSE pedido; se não puder resolvê-lo, escale (escalar_humano=true) em vez de repetir.`,
    ``,
    `Mensagem que NÃO pede resposta nem ação — agradecimento, "tudo certo", "ok, obrigado", confirmação final de algo já resolvido: encerrar=true, resposta VAZIA, escalar_humano=false. Não gaste uma palavra nem escale: não há nada para um humano resolver. Se houver QUALQUER pergunta, reclamação ou pendência no meio do agradecimento, encerrar=false e trate normalmente.`,
    ``,
